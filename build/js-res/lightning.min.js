...
lightning.admin={},lightning.admin.messageEditor={checkVars:function(){var a=$("#message_criteria_input_array").val();$.ajax({url:"/admin/mailing/messages",data:{action:"fields",criteria_list:a,message_id:$("#id").val()},dataType:"JSON",type:"GET",success:function(i){a=a.split(",");for(var t=0;t<a.length;t++)if(""!=a[t])for(var e in i.criteria){var r=i.criteria[e].criteria_id;if(i.criteria[e].criteria_id==a[t])for(var n in i.criteria[e].variables){var s=i.criteria[e].variables[n];if(0==$("#var_"+r+"_"+s).length){var l=i.criteria[e].values&&i.criteria[e].values[s]?i.criteria[e].values[s]:"";$("#message_criteria_box_"+a[t]).append('<div id="var_'+r+"_"+s+'" >'+s+': <input type="text" name="var_'+r+"_"+s+'" value="'+l+'"></div>')}}}},error:function(){alert("error")}})}},lightning.admin.messages={send:function(a){$("#message_status").html("Starting ...\n"),$(".mail_buttons").fadeOut();var i=0,t=this;$.ajax({url:"/admin/mailing/send?action=send-"+a+"&id="+lightning.vars.message_id,dataType:"text",data:{token:lightning.vars.token},type:"POST",stream:!0,xhrFields:{onprogress:function(a){var e=a.currentTarget.response;t.addContent("#message_status",e.substring(i)),i=e.length}},complete:function(){$(".mail_buttons").fadeIn()},error:function(){$(".mail_buttons").fadeIn()}})},addContent:function(a,i){var t=$(a);t.html(t.html()+i),t.animate({scrollTop:t.attr("scrollHeight")},500)}};
lightning.ajax={jqueryAjax:function(){},init:function(){this.jqueryAjax=jQuery.ajax,jQuery.ajax=this.call},success:function(r,e){if("HTML"==r.dataType)return void(r.user_success?r.user_success(e):lightning.dialog.showContent(e));if(e&&e.messages&&e.messages.length>0){lightning.dialog.clear();for(var s in e.messages)lightning.dialog.add(e.messages[s],"message");lightning.dialog.show()}return e&&e.errors&&e.errors.length?void lightning.ajax.error(r,e):e&&e.status&&"redirect"==e.status?void(document.location=e.location):void(r.user_success&&r.user_success(e))},error:function(r,e){if(lightning.dialog.clear(),void 0==e)lightning.dialog.add("Communication Error","error"),lightning.dialog.show();else if("string"==typeof e)lightning.dialog.add(e,"error"),lightning.dialog.show();else if(e.errors)for(var s in e.errors)lightning.dialog.add(e.errors[s],"error"),lightning.dialog.show();else lightning.dialog.add('There was an error loading the page. Please reload the page. If the problem persists, please <a href="/contact">contact support</a>.',"error"),e.hasOwnProperty("status")&&lightning.dialog.add("HTTP: "+e.status,"error"),e.hasOwnProperty("responseText")&&!e.responseText.match(/<html/i)&&lightning.dialog.add(e.responseText,"error"),lightning.dialog.show();r.user_error&&r.user_error(e)},call:function(r){r.user_error=r.error,r.user_success=r.success,r.success=function(e){lightning.ajax.success(r,e)},r.error=function(e){lightning.ajax.error(r,e)},lightning.ajax.jqueryAjax(r)}};
!function(){var a;lightning.cms={edit:function(a){lightning.htmleditor.initEditor(a),$("#"+a.replace(/^cms_/,"cms_edit_")).hide(),$("#"+a.replace(/^cms_/,"cms_save_")).show()},save:function(e){lightning.dialog.showLoader("Saving..."),lightning.htmleditor.deactivateEditor(e),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:e.replace(/^cms_/,""),token:lightning.vars.token,action:"save",content:$("#"+e).html()},success:function(a){$("#"+e.replace(/^cms_/,"cms_edit_")).show(),$("#"+e.replace(/^cms_/,"cms_save_")).hide(),lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){a.edit(e),lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initImage:function(){$(".cms_edit_image").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editImage(i)}),$(".cms_save_image").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_save_/,"");a.saveImage(i)}),$(".imagesCSS").keyup(function(){var a=$(this),e=a.attr("id").replace("_class",""),i=a.attr("name")+" "+a.val();$("#"+e).removeClass().addClass(i)})},editImage:function(a){$("#cms_save_"+a).show(),$("#cms_"+a+"_class").show(),lightning.fileBrowser.openSelect("lightning-cms",a)},imageSelected:function(a,e){$("#cms_"+e).attr("src",a)},saveImage:function(a){lightning.dialog.showLoader("Saving..."),$("#cms_save_"+a).hide(),$("#cms_"+a+"_class").hide(),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:a,class:$("#cms_"+a+"_class").val(),token:lightning.vars.token,action:"save-image",content:$("#cms_"+a).attr("src").replace(lightning.vars.cms.baseUrl,"")},success:function(){lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){$("#cms_save_"+a).show(),$("#cms_"+a+"_class").show(),lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initPlain:function(){$(".cms_edit_plain").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editPlain(i)}),$(".cms_save_plain").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_save_/,"");a.savePlain(i)})},editPlain:function(a){$("#cms_edit_"+a).hide(),$("#cms_save_"+a).show(),$("#cms_display_"+a).hide(),$("#cms_"+a).show()},savePlain:function(a){lightning.dialog.showLoader("Saving..."),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:a,token:lightning.vars.token,action:"save",content:$("#cms_"+a).val()},success:function(){$("#cms_edit_"+a).show(),$("#cms_save_"+a).hide(),$("#cms_"+a).hide(),$("#cms_display_"+a).html($("#cms_"+a).val()).show(),lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initDate:function(){$(".cms_edit_date").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editDate(i)}),$(".cms_save_date").on("click",function(e){var i=$(e.target).attr("id").replace(/^cms_save_/,"");a.saveDate(i)})},editDate:function(a){$("#cms_edit_"+a).hide(),$("#cms_save_"+a).show(),$("#cms_"+a).show()},saveDate:function(a){$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{id:a,key:$("#cms_key_"+a).val(),column:$("#cms_column_"+a).val(),table:$("#cms_table_"+a).val(),token:lightning.vars.token,action:"update-date",date_m:$("#cms_"+a+"_m").val(),date_d:$("#cms_"+a+"_d").val(),date_y:$("#cms_"+a+"_y").val()},success:function(a){var e="";for(var i in a.errors)e+=a.error[i]+" ";""==e&&(e="Could not save: Unknown error.")},error:function(){$("#cms_edit_"+a).show(),$("#cms_save_"+a).hide(),$("#cms_"+a).hide();var e=$("#cms_"+a+"_m").val()+"/"+$("#cms_"+a+"_d").val()+"/"+$("#cms_"+a+"_y").val();$("#date_"+a).text(e)}})}},a=lightning.cms}();
...
lightning.forms={init:function(){}};
!function(){var t;lightning.htmleditor={editors:{},init:function(){for(var i in lightning.vars.htmleditors)if("tinymce"==lightning.vars.htmleditors[i].editor_type)lightning.vars.htmleditors[i].setup=t.tinymceSetup,lightning.vars.htmleditors[i].browser&&(lightning.vars.htmleditors[i].file_browser_callback=t.imageBrowser),lightning.vars.htmleditors[i].startup&&t.initEditor(i);else if("ckeditor"==lightning.vars.htmleditors[i].editor_type){(function(i){CKEDITOR.scriptLoader.queue(CKEDITOR.getUrl("config.js"),function(){"function"==typeof CKEDITOR.editorConfig&&CKEDITOR.editorConfig(CKEDITOR.config),lightning.vars.htmleditors[i].hasOwnProperty("plugins")?lightning.vars.htmleditors[i].plugins=lightning.vars.htmleditors[i].plugins.replace(/\*/,CKEDITOR.config.plugins):lightning.vars.htmleditors[i].plugins=CKEDITOR.config.plugins,lightning.vars.htmleditors[i].hasOwnProperty("fullPage")&&lightning.vars.htmleditors[i].fullPage&&(lightning.vars.htmleditors[i].plugins=lightning.vars.htmleditors[i].plugins.replace("divarea","")),lightning.vars.htmleditors[i].startup&&t.initEditor(i)})})(i)}$(".html_editor_presave").closest("form").submit(function(){$(this).find(".html_editor_presave").each(function(){var i=$(this),n=$(this).prop("id").replace("save_","");"ckeditor"==lightning.vars.htmleditors[n].editor_type&&i.val(t.getContent(n))})})},getContent:function(i){if("ckeditor"==lightning.vars.htmleditors[i].editor_type)return t.editors[i].getData()},initEditor:function(i){"tinymce"==lightning.vars.htmleditors[i].editor_type?tinymce.init(lightning.vars.htmleditors[i]):"ckeditor"==lightning.vars.htmleditors[i].editor_type&&($("#"+i).attr("contenteditable","true"),t.editors[i]=CKEDITOR.inline(i,lightning.vars.htmleditors[i]))},deactivateEditor:function(i){if("tinymce"==lightning.vars.htmleditors[i].editor_type)for(var n in tinymce.editors)tinymce.editors[n].id==i&&tinymce.editors[n].remove();else"ckeditor"==lightning.vars.htmleditors[i].editor_type&&($("#"+i).attr("contenteditable","false"),t.editors[i].destroy())},tinymceSetup:function(t){t.addButton("columns",{type:"menubutton",text:"Columns",icon:!1,menu:[{text:"2 Columns",onclick:function(){t.insertContent('<div class="row"><div class="small-12 medium-6 column"><p>Column 1</p></div><div class="small-12 medium-6 column"><p>Column 2</p></div></div>')}},{text:"3 Columns",onclick:function(){t.insertContent('<div class="row"><div class="small-12 medium-4 column"><p>Column 1</p></div><div class="small-12 medium-4 column"><p>Column 2</p></div><div class="small-12 medium-4 column"><p>Column 3</p></div></div>')}},{text:"4 Columns",onclick:function(){t.insertContent('<div class="row"><div class="small-12 medium-3 column"><p>Column 1</p></div><div class="small-12 medium-3 column"><p>Column 2</p></div><div class="small-12 medium-3 column"><p>Column 3</p></div><div class="small-12 medium-3 column"><p>Column 4</p></div></div>')}}]}),t.addButton("yt",{type:"button",text:"YT",icon:!1,onclick:function(){t.insertContent('{{youtube id="" flex=true widescreen=true}}')}})},imageBrowser:function(t,i,n,e){return tinymce.activeEditor.windowManager.open({file:"/elfinder/elfinder.html?type=tinymce",title:"Image Browser",width:900,height:450,resizable:"yes"},{setUrl:function(i){e.document.getElementById(t).value=i}}),!1}},t=lightning.htmleditor}();
...
!function(){var e;lightning.page={edit:function(){$(".page_edit").show(),$(".page_edit_links").hide(),$("#page_display").html($("#save_page_display").val()),lightning.htmleditor.initEditor("page_display")},save:function(){$("#save_button").hide(),lightning.dialog.showLoader("Saving...");var a={page_id:$("#page_id").val(),token:$("#token").val(),action:"save",title:$("#page_title").val(),url:$("#page_url").val(),menu_context:$("#page_menu_context").val(),description:$("#page_description").val(),keywords:$("#page_keywords").val(),sitemap:$("#page_sitemap").is(":checked")?1:0,layout:$("#page_layout").val(),page_body:lightning.htmleditor.getContent("page_display")};$.ajax({url:"/page",type:"POST",dataType:"json",data:a,success:function(e){lightning.htmleditor.deactivateEditor("page_display"),$(".page_edit").hide(),$(".page_edit_links").show(),$("#page_display").attr("contentEditable","false"),$("#page_id").val(e.page_id),$("#page_url").val(e.url),document.title=e.title,$("#page_header").html(e.title),$("#save_page_display").val(e.page_body),$("#page_display").html(e.body_rendered),lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){lightning.dialog.clear(),lightning.dialog.add("The page could not be saved, please try again!","error"),e.edit()}})}},e=lightning.page}();
lightning.social={google:{signin:function(){lightning.require("https://apis.google.com/js/platform.js",function(){gapi.load("auth2",function(){var i=gapi.auth2.getAuthInstance()||gapi.auth2.init({client_id:lightning.vars.social.google.client_id});response=i.signIn().then(function(i){if(lightning.vars.social.authorize){for(var n in i)if("object"==typeof i[n]&&i[n].hasOwnProperty("access_token")){var t={access_token:i[n].access_token,created:i[n].first_issued_at,expires_in:i[n].expires_in};return void lightning.social.signinComplete("google","auth",JSON.stringify(t))}}else lightning.social.signinComplete("google","id",i.getAuthResponse().id_token)})})})},signout:function(){lightning.require("https://apis.google.com/js/platform.js",function(){gapi.load("auth2",function(){gapi.auth2.getAuthInstance()||gapi.auth2.init({client_id:lightning.vars.social.google.client_id});lightning.social.google.finalizeLogout()})})},finalizeLogout:function(){var i=gapi.auth2.getAuthInstance(),n=!1;i.signOut().then(function(){n=!0,window.location.href="/user?action=logout"}),setTimeout(function(){n||lightning.social.google.finalizeLogout()},500)}},facebook:{init:function(){},signin:function(){if(!lightning.vars.social.facebook.appid)return void console.log("Missing facebook app ID");FB.init({appId:lightning.vars.social.facebook.appid,cookie:!0,xfbml:!1,version:"v2.2"});var i={},n=lightning.get("social.facebook.scope");n&&(i.scope=n),FB.login(lightning.social.facebook.signinComplete,i)},signinComplete:function(i){"connected"===i.status?lightning.vars.social.authorize?lightning.social.signinComplete("facebook","auth",i.authResponse.accessToken):lightning.social.signinComplete("facebook","id",i.authResponse.signedRequest):console.log("could not sign in to facebook")},signout:function(){}},twitter:{signin:function(){window.location.href=lightning.vars.social.twitter.signin_url}},initShare:function(){$(".social-share").on("click","div",lightning.social.shareClick)},shareClick:function(){var i=$(this),n=i.closest(".social-share").data("url");i.is(".facebook")?lightning.social.sharePop("http://www.facebook.com/sharer.php?u="+n):i.is(".twitter")?lightning.social.sharePop("https://twitter.com/intent/tweet?url="+n):i.is(".google")?lightning.social.sharePop("https://plus.google.com/share?url="+n):i.is(".linkedin")&&lightning.social.sharePop("http://www.linkedin.com/shareArticle?mini=true&url="+n)},sharePop:function(i){var n=350,t=520,o=screen.height/2-n/2,e=screen.width/2-t/2;window.open(i,"sharer","top="+o+",left="+e+",toolbar=0,status=0,width="+t+",height="+n)},initLogout:function(i){$(".logout_button").on("click",function(n){n.preventDefault(),lightning.social[i].signout()})},initLogin:function(){$(".social-signin").on("click",function(){$(this).is(".google")&&lightning.social.google.signin(),$(this).is(".facebook")&&lightning.social.facebook.signin(),$(this).is(".twitter")&&lightning.social.twitter.signin()})},signinComplete:function(i,n,t){var o=lightning.get("social.signin_url");null==o&&(o="/user");var e=$('<form method="post" style="display:none">').attr("action",o);e.append('<input type="hidden" name="token" value="'+lightning.vars.token+'">');var s=lightning.get("social.login_redirect");s&&e.append('<input type="hidden" name="redirect" value="'+s+'">'),e.append('<input type="hidden" name="action" value="'+i+'-login">'),e.append($('<input type="hidden" name="'+n+'-token">').val(t)),e.submit()}};
lightning.splitTest={init:function(){if(lightning.vars.hasOwnProperty("splitTest"))for(var t in lightning.vars.splitTest)ga("send",{hitType:"event",eventCategory:t,eventAction:lightning.vars.splitTest[t],eventLabel:"Split Test"})}};
...
function reset_field_value(e){"undefined"!=typeof CKEDITOR.instances[e]?CKEDITOR.instances[e].setData(table_data.defaults[e]):"undefined"!=typeof("#"+e).val?$("#"+e).val(table_data.defaults[e]):$("#"+e).html(table_data.defaults[e])}lightning.table={init:function(){$(".add_image").on("click",lightning.table.clickAddImage),$(".linked_images").on("click",".remove",lightning.table.removeLinkedImage);var e=this,a=$("#table_search");a.keyup(function(){e.search(a)}),$("table.table_form_table").on("click",".remove-link",function(a){var t=$(a.target);e.removeLink(t.data("link"),t.data("link-item"))}).on("click",".add-link",function(a){e.addLink($(a.target).data("link"))})},click:function(e){var a=e.currentTarget.id;if("INPUT"!=e.target.tagName&&void 0!=table_data.rowClick)switch(table_data.rowClick.type){case"url":document.location=table_data.rowClick.url+a;break;case"action":document.location=lightning.table.createUrl(table_data.rowClick.action,a)}},createUrl:function(e,a){var t=[];if(t.push("action="+encodeURIComponent(e)),t.push("id="+encodeURIComponent(a)),table_data.table&&t.push("table="+encodeURIComponent(table_data.table)),table_data.parent_link&&t.push(table_data.parent_link+"="+encodeURIComponent(table_data.parent_id)),table_data.vars)for(var n in table_data.vars)null!=table_data.vars[n]&&t.push(n+"="+encodeURIComponent(table_data.vars[n]));return url="?"+t.join("&")},selectAll:function(e){$(".taf_"+e).prop("checked",$("#taf_all_"+e).is(":checked"))},searchTimeout:null,searchIndex:0,searchIndexDisplayed:0,search:function(e){var a=$(e).val();this.searchIndex++,clearTimeout(this.searchTimeout);var t=this;this.searchTimeout=setTimeout(function(){table_data.vars.ste=a,$.ajax({url:table_data.action_file,type:"GET",dataType:"json",data:{ste:a,i:t.searchIndex,action:"search"},success:function(e){e.d>t.searchIndexDisplayed&&(t.searchIndexDisplayed=e.d,$(".table_list").html(e.html))}})},500)},addLink:function(e){var a=$("#"+e+"_list"),t=a.val(),n=$("#"+e+"_list option:selected").text();if(0!=t||""!=n){var l=new RegExp("(^|,)"+t+","),i=$("#"+e+"_input_array");if(null==i.val().match(l)){i.val(i.val()+t+","),$("#"+e+"_list_container").append($('<div class="'+e+'_box table_link_box_selected" id="'+e+"_box_"+t+'">'+n+' <i class="remove-link fa fa-close" data-link="'+e+'" data-link-item="'+t+'"></i></div>'));var o=lightning.get("table.linkProcess");for(var s in o)lightning.getMethodReference(o[s])()}}},removeLink:function(e,a){$("#"+e+"_box_"+a).remove();var t=$("#"+e+"_input_array"),n=t.val(),l=new RegExp("[\\^,]"+a+",","i");n=n.replace(l,","),l=new RegExp("^"+a+",","i"),n=n.replace(l,""),t.val(n)},clickAddImage:function(e){var a=e.target.id.replace("add_image_","");CKFinder.popup({basePath:lightning.vars.table.links[a].web_location,chooseFiles:!0,chooseFilesOnDblClick:!0,onInit:function(e){e.on("files:choose",function(e){var t=e.data.files.first();lightning.table.addImageCallback(a,t.getUrl())}),e.on("file:choose:resizedImage",function(e){lightning.table.addImageCallback(a,e.data.resizedUrl)})}})},addImageCallback:function(e,a){$("#linked_images_"+e).append('<span class="selected_image_container"><input type="hidden" name="linked_images_'+e+'[]" value="'+a+'"><span class="remove">X</span><img src="'+a+'" /></span>')},removeLinkedImage:function(e){$(e.target).closest(".selected_image_container").remove()},autocomplete:function(){var e=$(this).attr("id");table_data.fields||(table_data.fields={},table_data.fields[e]={});var a=$(this).val();a.length>=2&&(!table_data.fields[e].last_fetch||table_data.fields[e].last_fetch!=a)&&(table_data.fields[e].last_fetch=a,$.ajax({url:table_data.action_file,type:"get",dataType:"json",data:{action:"autocomplete",field:e,st:table_data.fields[e].last_fetch},success:function(t){a==table_data.fields[e].last_fetch&&lightning.table.autocompleteDropdown(e,t.results)},error:function(){alert("error")}}))},autocompleteDropdown:function(e,a){$(".table_container").each(function(){$(this).attr("id")!="table_container_"+e?$(this).remove():$("#list_"+e).empty()}),0==$(".table_container").length&&$("#"+e).after("<div id='table_container_"+e+"' class='table_container'><div id='list_"+e+"'></div></div>");var t=0;for(var n in a)$("#list_"+e).append('<span id="'+e+"_"+n+'" >'+a[n]+"</span>"),t++;$("#list_"+e+" span").on("click",lightning.table.setAutocompleteSelection)},setAutocompleteSelection:function(e){var a=$(this).attr("id").split("_"),t=a.pop(),n=a.join("_");$("#table_container_"+n).remove(),$("#"+n).val(t)},deleteSubtable:function(e){var a=$(e).closest("div").attr("id").replace("subtable_",""),t=a.split("_");t=t[t.length-1],entry_id_table=a.replace("_"+t,""),parseInt(t)>0?$("#delete_subtable_"+entry_id_table).val($("#delete_subtable_"+entry_id_table).val()+t+","):(reg=new RegExp(Math.abs(t)+","),$("#new_subtable_"+entry_id_table).val($("#new_subtable_"+entry_id_table).val().replace(reg,""))),$("#subtable_"+entry_id_table+"_"+t).fadeOut(function(){$(this).remove()})},newSubtable:function(e){"undefined"==typeof new_subtables&&(new_subtables=Array()),"undefined"==typeof new_subtables[e]?new_subtables[e]=1:new_subtables[e]++,$("#subtable_"+e+"__N_").before($("<div class='subtable' id='subtable_"+e+"_-"+new_subtables[e]+"'></div>").html($("#subtable_"+e+"__N_").html().replace(/_N_/g,"-"+new_subtables[e]))),$("#new_subtable_"+e).val($("#new_subtable_"+e).val()+new_subtables[e]+",")},newPop:function(e,a,t){e.indexOf("?")>-1?window.open(e+"&action=pop&pf="+a+"&pfdf="+t+"&pop=1","New","width=400,height=500"):window.open(e+"?action=pop&pf="+a+"&pfdf="+t+"&pop=1","New","width=400,height=500")},returnPop:function(e){window.opener.$("#"+e.pf+"_list").append("<option value='"+e.id+"'>"+e.pfdf+"</option>").val(e.id),window.close()},export:function(e){var a=$('[name="table_search"]').val();console.log(e);var t=$(e).attr("href");""!=a&&(t+="&ste="+a),window.location=t}};
