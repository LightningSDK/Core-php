...
!function(){var a=lightning.cms={edit:function(a){lightning.htmleditor.initEditor("cms_"+a),$("#cms_edit_"+a).hide(),$("#cms_save_"+a).show()},save:function(e){lightning.dialog.showLoader("Saving..."),lightning.htmleditor.deactivateEditor("cms_"+e),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:e.replace(/^cms_/,""),token:lightning.vars.token,action:"save",content:$("#cms_"+e).html()},success:function(a){$("#cms_edit_"+e).show(),$("#cms_save_"+e).hide(),lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){a.edit(e),lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initImage:function(){$(".cms_edit_image").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editImage(t)}),$(".cms_save_image").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_save_/,"");a.saveImage(t)}),$(".imagesCSS").keyup(function(){var a=$(this),e=a.attr("id").replace("_class",""),t=a.attr("name")+" "+a.val();$("#"+e).removeClass().addClass(t)})},editImage:function(a){$("#cms_save_"+a).show(),$("#cms_"+a+"_class").show(),lightning.fileBrowser.openSelect("lightning-cms",a)},imageSelected:function(a,e){$("#cms_"+e).attr("src",a)},saveImage:function(a){lightning.dialog.showLoader("Saving..."),$("#cms_save_"+a).hide(),$("#cms_"+a+"_class").hide(),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:a,class:$("#cms_"+a+"_class").val(),token:lightning.vars.token,action:"save-image",content:$("#cms_"+a).attr("src").replace(lightning.vars.cms.baseUrl,"")},success:function(){lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){$("#cms_save_"+a).show(),$("#cms_"+a+"_class").show(),lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initPlain:function(){$(".cms_edit").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_edit_/,"");a.edit(t)}),$(".cms_save").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_save_/,"");a.save(t)}),$(".cms_edit_plain").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editPlain(t)}),$(".cms_save_plain").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_save_/,"");a.savePlain(t)})},editPlain:function(a){$("#cms_edit_"+a).hide(),$("#cms_save_"+a).show(),$("#cms_display_"+a).hide(),$("#cms_"+a).show()},savePlain:function(a){lightning.dialog.showLoader("Saving..."),$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{cms:a,token:lightning.vars.token,action:"save",content:$("#cms_"+a).val()},success:function(){$("#cms_edit_"+a).show(),$("#cms_save_"+a).hide(),$("#cms_"+a).hide(),$("#cms_display_"+a).html($("#cms_"+a).val()).show(),lightning.dialog.clear(),lightning.dialog.add("Saved!","message")},error:function(){lightning.dialog.clear(),lightning.dialog.add("The content could not be saved, please try again!","error")}})},initDate:function(){$(".cms_edit_date").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_edit_/,"");a.editDate(t)}),$(".cms_save_date").on("click",function(e){var t=$(e.target).attr("id").replace(/^cms_save_/,"");a.saveDate(t)})},editDate:function(a){$("#cms_edit_"+a).hide(),$("#cms_save_"+a).show(),$("#cms_"+a).show()},saveDate:function(a){$.ajax({url:"/admin/cms",type:"POST",dataType:"json",data:{id:a,key:$("#cms_key_"+a).val(),column:$("#cms_column_"+a).val(),table:$("#cms_table_"+a).val(),token:lightning.vars.token,action:"update-date",date_m:$("#cms_"+a+"_m").val(),date_d:$("#cms_"+a+"_d").val(),date_y:$("#cms_"+a+"_y").val()},success:function(a){var e="";for(var t in a.errors)e+=a.error[t]+" ";""==e&&(e="Could not save: Unknown error.")},error:function(){$("#cms_edit_"+a).show(),$("#cms_save_"+a).hide(),$("#cms_"+a).hide();var e=$("#cms_"+a+"_m").val()+"/"+$("#cms_"+a+"_d").val()+"/"+$("#cms_"+a+"_y").val();$("#date_"+a).text(e)}})}}}();
...
!function(){var e=lightning.table={init:function(){$(".add_image").on("click",e.clickAddImage),$(".linked_images").on("click",".remove",e.removeLinkedImage),$("table.table_form_table").on("click",".remove-link",function(t){var a=$(t.target);e.removeLink(a.data("link"),a.data("link-item"))}).on("click",".add-link",function(t){e.addLink($(t.target).data("link"))}),e.filterContainer=$(".filters"),e.filterContainer.on("change","select",e.search),e.filterSelect=$("#filters"),e.filterSelect.on("change",e.addNewFilter),e.filters=lightning.get("table.filters",[]),e.textSearch=$("#table_search"),e.textSearch.on("keyup",e.search)},addNewFilter:function(){var t=e.filterSelect.val();if(""!=t){e.filterSelect.val("");var a=$('<div class="filter">');a.data("name",t);for(var n in e.filters[t].options){if("select"==e.filters[t].options[n].type){var l=$("<select>");l.prop("name",n);for(var i in e.filters[t].options[n].options){var o=$("<option>");o.prop("value",i),o.html(e.filters[t].options[n].options[i]),l.append(o)}}a.append(l)}e.filterContainer.append(a)}},click:function(t){var a=t.currentTarget.id;if("INPUT"!=t.target.tagName&&void 0!=table_data.rowClick)switch(table_data.rowClick.type){case"url":document.location=table_data.rowClick.url+a;break;case"action":document.location=e.createUrl(table_data.rowClick.action,a)}},createUrl:function(e,t){var a=[];if(a.push("action="+encodeURIComponent(e)),a.push("id="+encodeURIComponent(t)),table_data.table&&a.push("table="+encodeURIComponent(table_data.table)),table_data.parent_link&&a.push(table_data.parent_link+"="+encodeURIComponent(table_data.parent_id)),table_data.vars)for(var n in table_data.vars)null!=table_data.vars[n]&&a.push(n+"="+encodeURIComponent(table_data.vars[n]));return url="?"+a.join("&")},selectAll:function(e){$(".taf_"+e).prop("checked",$("#taf_all_"+e).is(":checked"))},searchTimeout:null,searchIndex:0,searchIndexDisplayed:0,search:function(){var t=e.textSearch.val(),a={};e.filterContainer.find(".filter").each(function(){var e={},t=$(this);t.find("select").each(function(){e[this.name]=$(this).val()}),a[t.data("name")]=e}),e.searchIndex++,clearTimeout(e.searchTimeout),e.searchTimeout=setTimeout(function(){table_data.vars.ste=t,$.ajax({url:table_data.action_file,type:"GET",dataType:"json",data:{ste:t,i:e.searchIndex,action:"search",filter:a},success:function(t){t.d>e.searchIndexDisplayed&&(e.searchIndexDisplayed=t.d,$(".table_list").html(t.html))}})},500)},addLink:function(e){var t=$("#"+e+"_list"),a=t.val(),n=$("#"+e+"_list option:selected").text();if(0!=a||""!=n){var l=new RegExp("(^|,)"+a+","),i=$("#"+e+"_input_array");if(null==i.val().match(l)){i.val(i.val()+a+","),$("#"+e+"_list_container").append($('<div class="'+e+'_box table_link_box_selected" id="'+e+"_box_"+a+'">'+n+' <i class="remove-link fa fa-close" data-link="'+e+'" data-link-item="'+a+'"></i></div>'));var o=lightning.get("table.linkProcess");for(var s in o)lightning.getMethodReference(o[s])()}}},removeLink:function(e,t){$("#"+e+"_box_"+t).remove();var a=$("#"+e+"_input_array"),n=a.val(),l=new RegExp("[\\^,]"+t+",","i");n=n.replace(l,","),l=new RegExp("^"+t+",","i"),n=n.replace(l,""),a.val(n)},clickAddImage:function(t){var a=t.target.id.replace("add_image_","");CKFinder.popup({basePath:lightning.vars.table.links[a].web_location,chooseFiles:!0,chooseFilesOnDblClick:!0,onInit:function(t){t.on("files:choose",function(t){var n=t.data.files.first();e.addImageCallback(a,n.getUrl())}),t.on("file:choose:resizedImage",function(t){e.addImageCallback(a,t.data.resizedUrl)})}})},addImageCallback:function(e,t){$("#linked_images_"+e).append('<span class="selected_image_container"><input type="hidden" name="linked_images_'+e+'[]" value="'+t+'"><span class="remove">X</span><img src="'+t+'" /></span>')},removeLinkedImage:function(e){$(e.target).closest(".selected_image_container").remove()},autocomplete:function(){var t=$(this).attr("id");table_data.fields||(table_data.fields={},table_data.fields[t]={});var a=$(this).val();a.length>=2&&(!table_data.fields[t].last_fetch||table_data.fields[t].last_fetch!=a)&&(table_data.fields[t].last_fetch=a,$.ajax({url:table_data.action_file,type:"get",dataType:"json",data:{action:"autocomplete",field:t,st:table_data.fields[t].last_fetch},success:function(n){a==table_data.fields[t].last_fetch&&e.autocompleteDropdown(t,n.results)},error:function(){alert("error")}}))},autocompleteDropdown:function(t,a){$(".table_container").each(function(){$(this).attr("id")!="table_container_"+t?$(this).remove():$("#list_"+t).empty()}),0==$(".table_container").length&&$("#"+t).after("<div id='table_container_"+t+"' class='table_container'><div id='list_"+t+"'></div></div>");var n=0;for(var l in a)$("#list_"+t).append('<span id="'+t+"_"+l+'" >'+a[l]+"</span>"),n++;$("#list_"+t+" span").on("click",e.setAutocompleteSelection)},setAutocompleteSelection:function(e){var t=$(this).attr("id").split("_"),a=t.pop(),n=t.join("_");$("#table_container_"+n).remove(),$("#"+n).val(a)},deleteSubtable:function(e){var t=$(e).closest("div").attr("id").replace("subtable_",""),a=t.split("_");a=a[a.length-1],entry_id_table=t.replace("_"+a,""),parseInt(a)>0?$("#delete_subtable_"+entry_id_table).val($("#delete_subtable_"+entry_id_table).val()+a+","):(reg=new RegExp(Math.abs(a)+","),$("#new_subtable_"+entry_id_table).val($("#new_subtable_"+entry_id_table).val().replace(reg,""))),$("#subtable_"+entry_id_table+"_"+a).fadeOut(function(){$(this).remove()})},newSubtable:function(e){"undefined"==typeof new_subtables&&(new_subtables=Array()),"undefined"==typeof new_subtables[e]?new_subtables[e]=1:new_subtables[e]++,$("#subtable_"+e+"__N_").before($("<div class='subtable' id='subtable_"+e+"_-"+new_subtables[e]+"'></div>").html($("#subtable_"+e+"__N_").html().replace(/_N_/g,"-"+new_subtables[e]))),$("#new_subtable_"+e).val($("#new_subtable_"+e).val()+new_subtables[e]+",")},newPop:function(e,t,a){e.indexOf("?")>-1?window.open(e+"&action=pop&pf="+t+"&pfdf="+a+"&pop=1","New","width=400,height=500"):window.open(e+"?action=pop&pf="+t+"&pfdf="+a+"&pop=1","New","width=400,height=500")},returnPop:function(e){window.opener.$("#"+e.pf+"_list").append("<option value='"+e.id+"'>"+e.pfdf+"</option>").val(e.id),window.close()},export:function(e){var t=$('[name="table_search"]').val();console.log(e);var a=$(e).attr("href");""!=t&&(a+="&ste="+t),window.location=a},resetField:function(e){"undefined"!=typeof CKEDITOR.instances[e]?CKEDITOR.instances[e].setData(table_data.defaults[e]):"undefined"!=typeof("#"+e).val?$("#"+e).val(table_data.defaults[e]):$("#"+e).html(table_data.defaults[e])}}}();
