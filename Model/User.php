	...
	function __construct($email = ""){
		...
		if($email != ""){
			...
			if(isset($_COOKIE['sess'])){
				...
				$session = Database::getInstance()->assoc1("SELECT * FROM session LEFT JOIN user USING (user_id) WHERE session_key = '{$_COOKIE['sess']}' AND session_ip=INET_ATON('{$_SERVER['REMOTE_ADDR']}') AND session_type=0");
				if($session['email']!=""){
					//if the last action has been more than 5 minutes then log them out
					if($session['last_active'] < (time() - $this->sess_expire_time)){
						// session expired
            Database::getInstance()->query("DELETE FROM session WHERE session_id={$session['session_id']}");
			...
			// CHECK IF THIS IS A MOBILE SESSION
				...
				$session = Database::getInstance()->assoc1("SELECT * FROM session LEFT JOIN user USING (user_id) WHERE session_key = '{$_REQUEST['sess']}' AND session_type=1");
				if($session['email']!=""){
...
/*
	public function is_admin($office_id){
		global $db;
		$db->verbose = true;
		$site_access = $db->assoc1("SELECT * FROM office_employee WHERE user_id={$this->id} AND office_id={$office_id}");
		$access_bits = strrev(decbin($site_access['access_options']));
		if($access_bits[0] == 1)
			return true;
		return false;
	}
...
/*
	public function is_user_case($case_id){
		global $db;
		return($db->check("SELECT * FROM office_case_user WHERE user_id = {$this->id} AND case_id = {$case_id}"));
	}
*/
	...
	function load_info($force = false){
		if(!isset($this->details) || $force){
			$this->details = Database::getInstance()->assoc1("SELECT * FROM user WHERE user_id = {$this->id}");
    }
	}
